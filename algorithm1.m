...
if Algorithm_in.Struct_list_boat.list_boat(Algorithm_in.idx_boat).num_mes==1
    ...
    mmsi=boat.mmsi;
    nb_error_lat=boat.nb_error_lat;
    nb_error_lon=boat.nb_error_lon;
    ...
    R=Algorithm_in.Kalman.R;
    sigma_w_lon=Algorithm_in.Kalman.sigma_w;
    sigma_w_lat=Algorithm_in.Kalman.sigma_w;
...
%     K_p latitude
    ...
    S_lat=(R+C*P_lat_pred*transpose(C));
...
%     K_p latitude
    ...
    S_lon=(R+C*P_lon_pred*transpose(C));

%     Controlled of the latitude consistency
    H_lat=P_lat_pred*transpose(C)*inv((R+C*P_lat_pred*transpose(C)));
    ...
    if (abs(inno_lat)<5*sqrt(S_lat))
        ...
        flag1=0;
        nb_error_lat=nb_error_lat+1;
    end
    
%     Controlled of the longitude consistency
    H_lon=P_lon_pred*transpose(C)*inv((R+C*P_lon_pred*transpose(C)));
...
%     we recorde parameters of the algorithm
    boat.NTR=NTR;
    boat.nb_error_lat=nb_error_lat;
    boat.nb_error_lon=nb_error_lon;
